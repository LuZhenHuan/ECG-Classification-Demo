...
class TestDataset(Data.Dataset):
    def __init__(self):
        a=np.array(h5py.File('/home/lu/code/pytorch/data1_60.mat')['tej'])
        #b=np.array(h5py.File('/home/lu/code/pytorch/data_dir/data11.mat')['train_data1'])
        self.data = np.transpose(a)
...
class CNN(nn.Module):
    ...
    def forward(self, x): 
        ...
        cnn_out = F.max_pool1d(F.leaky_relu(self.conv1(x)),7)
        cnn_out = F.dropout(cnn_out,p = 0.25)
        cnn_out = F.max_pool1d(F.leaky_relu(self.conv2(cnn_out)),7)
        cnn_out = F.dropout(cnn_out,p = 0.25)
