...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.MaxP2 = nn.MaxPool2d(7,1,padding = 3)
        self.conv5 = nn.Conv2d(52,1,(5,5),padding = 2)
        #self.up = nn.upsample()
    def forward(self, x):

        cnn_out = self.conv1(x)
        #cnn_out = F.max_pool2d(cnn_out,(2,2),2)
        #cnn_out = F.upsample(cnn_out, scale_factor = 2)
        cnn_out = self.MaxP1(cnn_out)
        cnn_out = self.maxout(cnn_out)

        t1 = self.conv2(cnn_out)
        t2 = self.conv3(cnn_out)
        t3 = self.conv4(cnn_out)
        t4 = self.MaxP1(cnn_out)
        
        cnn_out = torch.cat((t1,t2,t3,t4),1)
        cnn_out = self.MaxP2(cnn_out)
        
        cnn_out = F.relu6(self.conv5(cnn_out))
...
#################################################################
def trainIters(cnn, epoch, learning_rate=0.005):
...
torch.save(cnn, '/home/lu/code/data/DeHaze_4.pkl')
torch.save(cnn.state_dict(), '/home/lu/code/data/DeHaze_params_4.pkl')